...
; int  laodWordKernel(int addr)
_loadWordKernel:
    push bp
    mov bp, sp
    mov bx, ds ; copy ds ot bx
    mov cx, ss ; copy ss ot cx
    mov di, [bp+4]; store addr in dx
    mov ax, #0x1000
    mov ds, ax ; store seg 0x10000 in ds
    mov ss, ax ; store seg 0x10000 in ds
    xor ax, ax ; zero upper 8-bit of ax
    mov ax, word ptr [di] ; store char at lower 8-bit of ax
...
_loadCharKernel:
    ...
    mov ds, ax ; store seg 0x10000 in ds
    mov ss, ax ; store seg 0x10000 in ds
    xor ah, ah ; zero upper 8-bit of ax
    mov al, byte ptr [di] ; store char at lower 8-bit of ax
    mov ds, bx ; restore ds
    mov ss, cx ; restore ss
    mov sp, bp
    pop bp
